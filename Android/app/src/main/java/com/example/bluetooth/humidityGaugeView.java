...
public class humidityGaugeView extends View {
    ...
    public humidityGaugeView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }
    ...
    private void drawGaugeForeground(Canvas canvas) {
        ...
        // Create a path for the filled-in part
        ...
        SweepGradient sweepGradient = new SweepGradient(centerX, centerY,
                new int[] {0xFFFF0000, 0xFFFFA500, 0xFFFFFF00, 0xFF86DC3D, 0xFF0080FE, 0xFFB200ED},
                new float[] {0.05f, 0.15f, 0.3f, 0.45f, 0.65f, 0.9f});
    ...
    public void setValue(float value) {
        ...
        invalidate(); // Basically refreshes the screen
    }
    ...
    private void drawNeedle(Canvas canvas) {
        ...
        needlePath.close(); // Close the path to form a triangle

        needlePaint.setStrokeWidth(2);
        needlePaint.setStyle(Paint.Style.FILL_AND_STROKE);
        canvas.drawPath(needlePath, needlePaint);

        // Draw a black dot at the center (for mounting screw)
        Paint centerDotPaint = new Paint();
        centerDotPaint.setColor(0xFF000000);
